...
ClientGUI::ClientGUI(SDL_Surface* Surface, bool fullscreen)
{
	...
	SDL_ShowCursor(0);
	SDL_EnableUNICODE(1);
	titleFont = new CFont(FONT_TITLEFONT,FONT_TITLESIZE);
	normalFont = new CFont(FONT_NORMALFONT,FONT_NORMALSIZE);
	tinyFont = new CFont(FONT_TINYFONT,FONT_TINYSIZE);
...
bool ClientGUI::gameover_events()
{
	...
	while(SDL_PollEvent(&e)) {
		switch(e.type){
			...
			break;
			case SDL_KEYDOWN:
				if(e.key.keysym.sym==START_KEY || e.key.keysym.sym==ABORT_KEY)
					return false;
			break;	
		}
	}
...
void ClientGUI::updateScoreboard(signed char* scores, signed char* loads)
{
	...
	if(state==CSTATE_RUNNING||state==CSTATE_ENDOFROUND) {
		...
		SDL_FillRect(surface,&r,INGAME_BG_COLOR);
		unsigned char ranks[MAX_PLAYERS];
		getRanks(ranks);
		int cur_y = 0;
		for(int j=0;j<MAX_PLAYERS;j++){
...
{
	if(state != CSTATE_RUNNING) return;
	SDL_FillRect(surface,0,INGAME_BG_COLOR);
	SDL_Rect r;
	r.x = xoff;
	r.y = yoff;
	r.w = width;
...
void ClientGUI::lobby_paint()
{
	...
	//netLobby = true; // DEBUG
	if(netLobby) {
		aux::gfx_DrawRect(surface,795,419,210,279,0xFF0000);
		...
		aux::gfx_DrawRect(surface,15,419,781,245,0xFF0000);
		...
		aux::gfx_DrawRect(surface,15,663,781,35,0xFF0000);
