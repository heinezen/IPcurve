...
CControl::CControl(CControl* o,SDL_Surface* t)
{
	...
	if(target==0){
		if(owner==0){
			...
			if(cfg.c_fullscreen)
				target = SDL_SetVideoMode(SCREENWIDTH,SCREENHEIGHT,SCREENBPP,SDL_SWSURFACE|SDL_FULLSCREEN);
			else
			#endif
				target = SDL_SetVideoMode(SCREENWIDTH,SCREENHEIGHT,SCREENBPP,SDL_SWSURFACE);
			//#endif
			TTF_Init();
			resetSDL();
...
CMenu::CMenu(CControl* o,SDL_Surface* t) : CControl(o,t),
	titleFont(FONT_TITLEFONT,FONT_TITLESIZE)
{
...
CPushButton::CPushButton(CControl* o, int x, int y, std::string Caption, int fontsize, SDL_Surface* t) : CControl(o,t),
	font(FONT_MENUBUTTONFONT,fontsize)
{
...
CTextEdit::CTextEdit(CControl* o, int x, int y, int w, std::string Text, bool number, int fontsize, SDL_Surface* t) : CControl(o,t),
	font(FONT_CONTROLFONT,fontsize)
{
	text = Text;
...
CTooltip::CTooltip(CControl* o, int w, std::string Title, std::string Text, int title_fontsize, int text_fontsize, SDL_Surface* t) : CControl(o,t),
		title_font(FONT_CONTROLFONT,title_fontsize),
		text_font(FONT_CONTROLFONT,text_fontsize)		
{
	visible = false;
	title = Title;
	text = Text;
...
void CTooltip::show(int x, int y)
{
	...
	visible = true;
	repaint();
	printf("making tooltip visible at %d,%d - size %d,%d\n",x,y,w,h);
}
...
void CTooltip::real_paint(SDL_Surface* target)
{
	int cur_y;
	// Global Outline (Red) 
	SDL_FillRect(target,0,0xFF0000);
	cur_y = 1;
	// Title Bar
	aux::gfx_FillRect(target,1,cur_y,w-2,TopSpace + title_font.getHeight() + TitleSpace/2,0xFF5500);
	title_font.render_to(target,LeftSpace,cur_y+TopSpace,0,0,0,title);
	cur_y += title_font.getHeight() + TitleSpace/2 + TopSpace;
	// Text Box
	aux::gfx_FillRect(target,1,cur_y,w-2,h-cur_y-1,0xFF9900);
	cur_y += TitleSpace/2;
	int line_h = text_font.getHeight();
	string_list lines = text_font.wordWrap(text, w - (RightSpace+LeftSpace));
	for (unsigned int i=0;i<lines.size();i++) {
		text_font.render_to(target,LeftSpace,cur_y,0,0,0,lines[i]);
		cur_y += line_h;
	}
	/*
	...
	while(txt.length()>0) {
		...
		while(txt.length()>0) {
			...
			if(word == "\n ") {
				...
				int line_w_new = line_w + text_font.textWidth(word);
				if(line_w_new > w) {
					txt = word + txt;
					break;
				} else {
...
{
	aux::gfx_DrawRect(target,x,y+TAB_HEIGHT,w,h-TAB_HEIGHT,0xFF0000);
	int cur_x = x;
	int cur = 0;
	std::vector<CControl*>::iterator it;
	for(it = childs.begin(); it != childs.end(); it++){
		...
		aux::gfx_DrawRect(target,cur_x,y,cur_tab_w,TAB_HEIGHT,0xFF0000);
		if(activeTab==cur) 
			aux::gfx_FillRect(target,cur_x+1,y+1,cur_tab_w-2,TAB_HEIGHT,0xFFFF00);
		else
			aux::gfx_FillRect(target,cur_x+1,y+1,cur_tab_w-2,TAB_HEIGHT-2,0xFF7F00);
...
{
	aux::gfx_DrawRect(target,x,y,w,h,0xFF0000);
	aux::gfx_FillRect(target,x+1,y+1,w-2,h-2,0x000000);
	int colW;
	int curX = 0;
	for(int c=0;c<colCount;c++) {
		...
		aux::gfx_FillRect(target,x+curX+1,y+1,colW-1,titleH-2,0xFF7F00);
		font.render_to(target,x+curX+3,y,0,0,0,titles[c]);
		curX += colW;
		if(curX >= w)
			continue;
		aux::gfx_FillRect(target,x+curX,y,1,h,0xFF0000);
	}
	...
	aux::gfx_DrawRect(target,x,y,w,titleH,0xFF0000);
...
CPopup::CPopup(CControl* o, int w, int h, std::string title, int fontsize, SDL_Surface* t) : CMenu(0,t?t:o->getTarget()),
	title_font(FONT_MENUFONT,fontsize)
{
	caption = title;
	this->w = w;
	this->h = h;
...
{
	aux::gfx_FillRect(target,x,y,w,h,0x000000);
	aux::gfx_DrawRect(target,x,y,w,h,0xFFFF00);
	int title_h = title_font.getHeight();
	int title_w = title_font.textWidth(caption);
	aux::gfx_DrawRect(target,x,y,w,title_h,0xFFFF00);
	aux::gfx_FillRect(target,x+1,y+1,w-2,title_h-2,0x00007F);
