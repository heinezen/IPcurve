...
void ClientFindServers(CControl* sender,void* data)
{
	...
	for(unsigned int i=0;i<servers.size();i++) {
		...
		if(servers[i].players == -1)
			row.push_back("?");
		else
			row.push_back(aux::str_fromInt(servers[i].players));
		if(servers[i].ping == -1)
			row.push_back("?");
		else
			row.push_back(aux::str_fromInt(servers[i].ping));
		if(servers[i].pass)
...
//	new CPushButton(, 630, 80, CCapt::CAPT_SERVERS); --- REMOVE CCapt::CAPT_SERVERS
	...
	c.push_back(CCapt::CAPT_SERVERPLAYERS);
	c.push_back(CCapt::CAPT_SERVERPING);
	...
	cols->push_back(58);
	//CTable* table = new CTable(this,555,200,450,500,c);
	serverlist_meta = new CTable(tabInternet,10,110,1004,500,c);
	...
	cols->push_back(58);
	//CTable* table = new CTable(this,555,200,450,500,c);
	serverlist_lan = new CTable(tabLAN,10,110,1004,500,c);
...
//	serverlist->setOnElementClick(EnterServer,this);
	...
	// Warning
	//CPopupMessage m(this,"TODO","Dieser Teil wird gerade berarbeitet. Raus hier.");
	//m.execute();
	//requestClose();
}
