...
class CControl{
	public:
		...
		virtual void repaint(bool flip = true);
		void resetSDL();
		void cleanSDL();
...
class CMenu : public CControl {
	public:
		...
		CFont titleFont;
		void paintElement(CControl* ctrl);
		void unpaintElement(CControl* ctrl);
	protected:
		std::vector<CControl*> repaintQueue;
		int gl_shift;
		bool showTitle;
		std::string caption;
...
class CTooltip : public CControl {
	...
	/*	GLuint buffer_tex;
		GLfloat buffer_texcoords[4];
		GLuint back_tex;
		GLfloat back_texcoord[4];
	*/	
		CFont title_font;
		CFont text_font;
		virtual void paint(SDL_Surface* target);
