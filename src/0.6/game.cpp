...
void Game::startRound(int timeout, int wantedDelay)
{
	...
	for(int i=0;i<MAX_PLAYERS&&state==STATE_RUNNING&&(playersNow>1||(playersNow==1&&players==1));i++){
		if(snakes[i]==0)
			continue;
		mess.x[i] = snakes[i]->getX();
		mess.y[i] = snakes[i]->getY();
	...
	while((playersNow>1||(playersNow==1&&players==1))&&(time<(timeout*1000)||timeout==0)&&state==STATE_RUNNING){
		...
		server->update(wantedDelay-5-delta);
		for(int i=0; i<MAX_PLAYERS; i++){
			if(snakes[i] == 0){
				mess.x[i] = -1;
				mess.y[i] = -1;
				...
				if(mapcoll||wallcoll){
					...
					sprintf(t,"%d",i);
					std::string s = t;
					server->broadcast_chat_msg(CCapt::MSG_LOOSE_PRE+snakes[i]->getController()->name+CCapt::MSG_LOOSE_MID+s+CCapt::MSG_LOOSE_POST);
					playersNow--;
					updateScores(true);
				}
